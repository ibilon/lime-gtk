...
value hx_GdkWindow_getMouseInfo_x (value handle)
{
	GdkWindow* window = (GdkWindow*)(intptr_t)val_float(handle);

	int x;
	gdk_window_get_pointer (window, &x, NULL, NULL);

	return alloc_int(x);
}
DEFINE_PRIM (hx_GdkWindow_getMouseInfo_x, 1);


value hx_GdkWindow_getMouseInfo_y (value handle)
{
	GdkWindow* window = (GdkWindow*)(intptr_t)val_float(handle);
...
DEFINE_PRIM (hx_GdkWindow_getMouseInfo_y, 1);


value hx_GdkWindow_getMouseInfo_state (value handle)
{
	GdkWindow* window = (GdkWindow*)(intptr_t)val_float(handle);

	GdkModifierType state;
	gdk_window_get_pointer (window, NULL, NULL, &state);

	return alloc_int(state);
}
DEFINE_PRIM (hx_GdkWindow_getMouseInfo_state, 1);
